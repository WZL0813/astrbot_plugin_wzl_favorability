...
class FavorManager:
    ...
    def _init_config(self, config: AstrBotConfig):
        ...
        self.config = config
        # 基础配置
        self.black_threshold = config.get("black_threshold", 10)
        self.min_favor_value = config.get("min_favor_value", -1000)
        self.max_favor_value = config.get("max_favor_value", 10085)
        self.black_favor_limit = config.get("black_favor_limit", -900)
...
@register("astrbot_plugin_wzl_favorability", "WZL0813", "好感度管理", "1.2.0")
        ...
        try:
            if cmd == "好感度":
                if target and value is not None:
                    clamped_value = max(-1000, min(10086, int(value)))
